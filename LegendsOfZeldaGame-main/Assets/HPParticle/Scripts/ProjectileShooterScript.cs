...
public class ProjectileShooterScript : MonoBehaviour
{
    ...
    void Start()
    {
        Rosa = gameObject.transform.Find("Player").gameObject;
    }
    ...
    void FixedUpdate()
    {
        if (Time.time - LastShotTime >= Rate)//determine if a shot should be fired
        {
            ...
            GameObject ThisProjectile = Instantiate(Projectile, Rosa.transform.position, Rosa.transform.rotation) as GameObject;
